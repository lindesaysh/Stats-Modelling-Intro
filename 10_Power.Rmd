...
Imagine there is a new fuel additive that is expected to increase the fuel efficiency for vehicles. The underlying fuel efficiency with the standard fuel is assumed to be 25 mpg. It is thought an improvement of at least 3\% in fuel efficiency would be substantial enough for the additive to be taken to market. A trial is planned with the intention of detecting whether fuel efficiency with the additive has increased from 25 to 6 mpg (i.e. a 3\% increase) or even more. 
...
x <- seq (17,32,0.01)
 ...
 plot (x, dnorm (x,25,2.4), col=1, xlab="mpg", type="l", ylab="Density")  
 abline (h=0)
 abline (v=25, lty=2)
  lines (c(25.72,25.72), c(0,  dnorm (25.72, 25, 2.4)), col=2)
 
polygon (c(25.72, 35, seq(35,25.72,-0.01),25.72), c(0,0,dnorm (seq(35,25.72,-0.01), 25, 2.4)   ,0), density=3, col=2)
...
x <- seq (17,32,0.01)
 ...
 polygon (c(25.72, 35, seq(35,25.72,-0.01),25.72), c(0,0,dnorm (seq(35,25.72,-0.01), 25, 2.4)   ,0), density=3, col=2)
text (28, 0.1, "Type I error region", col=2)
 lines (x, dnorm (x,26,2.4), col=5, type="l")  
 abline (v=26, lty=2, col=5)
  lines (c(25.72,25.72), c(0,  dnorm (25.72, 26, 2.4)), col=5)
 polygon (c(25.72, -15, seq(-15,25.72,0.01),25.72), c(0,0,dnorm (seq(-15,25.72,0.01), 26, 2.4)   ,0), density=4, col='purple')
text (23, 0.01, "Type II error region", col='purple')
```
