...
<!-- LB update including eqn numbers- 12/01/2022 -->
...
x <- seq(-6, 6, by=0.005)
y <- dunif(x, -5, 5)
plot(x,y, xlim=c(-6,6), type="l", ylab="Density", xlab="Temperature (X)", lwd=2)
# Create polygon
#polygon(c(-5,-5,5,5,-5), c(0,0.1,0.1,0,0), fillOddEven=T)
...
(ref:pdfnormex1) Examples of normal distributions with different values of $\mu$ and $\sigma^2$; $N(0,4)$ (black line), $N(0,25)$ (blue dashes), $N(10, 25)$ (black dots).
...
lines(x, y_0_5, lwd = 2, col = 'blue', lty=2)
lines(x, y_2_5, lwd = 2, lty=3)
``` 
...
__Example__ Assume that IQ (intelligent quotient) is distributed as $N(100,15^2)$. What is the IQ for someone at the 99th percentile i.e. what is $q$ such that $Pr(X \le X q)=0.99$?
...
__Q6.3__ __a.__  Using equation \@ref(rvtransexp), let $a=0$, $b=1$ and $c=2$,
