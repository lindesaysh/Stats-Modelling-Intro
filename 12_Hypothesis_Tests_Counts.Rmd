...
$$x_{stat}^2 = \sum_{\textrm{all cells}} \frac{(\textrm{observed count} - \textrm{expected count})^2}{\textrm{expected count}}$$
This formula is frequently abbreviated to:

$$x_{stat}^2 = \sum_{\textrm{all cells}} \frac{(\textrm{O} - \textrm{E})^2}{\textrm{E}}$$
...
$$x_{white}^2 = \frac{(21249 - 21636.46)^2}{21636.46}= 6.938$$

The chi-square contributions are calculated for all ethnic groups (Table \@ref(tab:pupilgroup4)).  
...
The chi-square values for each group are added together to give the overall chi-square test statistic.
$$x_{stat}^2 = 6.939 + 0.1432 + 215.14 + 77.37 + 631.3 = 930.9$$

As with other hypothesis tests, the larger the test statistic, the stronger the evidence against $H_0$. Therefore, we wish to know if this test statistic (i.e. $x_{stat}^2 = 930.9$) is considered large, if the null hypothesis is true. To determine this, we compare it to a reference distribution; not surprisingly, for a chi-square test, the reference distribution is a chi-square distribution. The chi-square distribution is indexed by one parameter, the degrees of freedom, found from (for a one-way table):
...
We can go further and investigate what gave rise to such a large test statistic; the chi-square components for the Black and Other groups were particularly large. The observed number in the Other group was 272 pupils, but according to the census, we expected 67 pupils in this group, hence this large discrepancy led to the large chi-square component. Thus, pupils identified as Other and Black were over-represented in the sample compared to the population. This may be due to biased sampling or changing demographics in the population. 

### Doing this in R

To perform a chi-square test in R, we need to provide the observed values and the proportions under the null hypothesis (note, the proportions need to sum to 1). 
...
Unfortunately, the test statistic object does not contain the chi-square components but these can easily be calculated:
...
As before, the test statistic ($x_{stat}^2$) is given by 
...
$$x^2_{stat} = 4.835 + 0.169 + 8.084 + 6.273 + 0.220 + 10.489 = 30.07$$

We need to compare this to a chi-square reference distribution; the degrees of freedom are found from:
...
If we examine the chi-square components (Table \@ref(tab:voters4)), we see that the largest components were for Republicans and then Democrats. There were less female Republicans and more male republicans than expected given the null hypothesis; the converse was true for Democrats. 

### Doing this in R

The key to performing a chi-square test of independence is getting the data into the correct matrix form and so it is useful to print it out before going ahead with the test. 
...
Note that for 2 $\times$ 2 tables, a correction is applied by default in the `chisq.test` function. The reason for this is that we assume a discrete distribution can be approximated by continuous distribution (i.e. the chi-square distribution). To account for this approximation, an adjustment is made to the formula to calculate $x_{stat}^2$ which makes the test statistic smaller, and thus the corresponding $p$-value will be larger.  
...
__Q12.1__ __c.__ The chi-square test statistic is given by:
...
__Q12.2__ This question calls for a chi-square test of independence. 
...
Summing the chi-square values gives the test statistic:
