...
__Q5.8__ Use the PMF values shown in section \@ref(binomR) for throwing four darts (i.e. the output from R) and equation \@ref(discexpx) to confirm that the expected number of bull's eye the player could be expected to throw is 1. Use equation \@ref(discvarx) to confirm that the variance is 0.75 and hence calculate the standard deviation.
...
dpois(x=0, lambda=4) + dpois(x=1, lambda=4) + 
  dpois(x=2, lambda=4) + dpois(x=3, lambda=4) 
```
