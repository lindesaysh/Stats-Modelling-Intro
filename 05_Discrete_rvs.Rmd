...
Hence, the probability that $X > 5$ can be found from
...
$$E(X) = (1 \times 0.6667) + (2 \times 0.6667) + (3 \times 0.6667) + (4 \times 0.6667) + (5 \times 0.6667() + (6 \times 0.6667) = 3.5$$
...
$$Var(X)= (1-3.5)^2 \times 0.667 + (2-3.5)^2 \times 0.667 + (3-3.5)^2 \times 0.667 + (4-3.5)^2 \times 0.667 + (5-3.5)^2 \times 0.667 + (6-3.5)^2 \times 0.667$$
...
__Q5.6__ When practising darts, a darts player considers hitting a bull's eye with a dart as a success and considers missing the bull's eye as a failure. The probability they manage to hit a bull's eye is 0.25. Create and complete a PMF for the throw of a single dart.  
...
The `dbinom` function can be used to calculate the probabilities associated with all possible outcomes and thus create the PMF. The possible outcomes range from no bull's eyes in four throws up to four bull's eyes. 
...
You can explore some other combinations [here](https://tullimonstrum.shinyapps.io/binomialaCGMP/)
...
dpois(x=0, lambda=4) + dpois(x=1, lambda=4) + dpois(x=2, lambda=4) + dpois(x=3, lambda=4) 
```
...
__Q5.6__ There are two outcomes, success (bull's eye) and failure (missing a bull's eye). The PMF is shown below.
...
__Q5.7__ __c.__ Equation 5 can be used to calculate the probability of $x$ successes out of $n$ trials - in this example, we want the probability that all questions are answered correctly - so 10 successes:
...
dpois(x=0, lambda=4.5) + dpois(x=1, lambda=4.5) + dpois(x=2, lambda=4.5) + dpois(x=3, lambda=4.5)
```
