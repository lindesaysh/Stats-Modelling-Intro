...
For each phase we have the number of successful observations and the number of observations in total (Table \@ref(tab:phasetab)). <br/>  
<caption> (\#tab:phasetab) Number of presence (1) or absence (0) observations in phases A, B and C in the building of a windfarm. </caption>
...
pander::pander(presabsphase, caption="")
```  
<br/> 
...
While the number of successes from the number of trials is assumed to be a binomial random variable, the sample proportions (estimates for $p$; $\hat{p}$) are normally distributed about the true (underlying) population proportion if the number of trials is __large__. Therefore, confidence intervals (CIs) for proportions are constructed in a similar way to CIs for large samples of normal data. For large samples, the sample porportion is approximately normally distributed: 
...
<caption> (\#tab:compwilson) Comparison of Wilson and asymptotic confidence intervals </caption>
```{r compwilson, echo=FALSE, message=FALSE, warning=FALSE}
require(Hmisc)
pander::pander(binconf(1,10, method='all')[-1,])
```
...
pander::pander(binconf(100, 1000,method='all')[-1,])
```
...
<caption> (\#tab:compphases) Comparison of Wilson confidence intervals for the windfarm phases</caption>
...
+ We do this using the normal distribution as a reference distribution because it turns out that when there are no differences across phases, the test statistic has a normal distribution with $\mu=0$ and $\sigma=1$. This distribution is known as the __standard normal distribution__ or __$z$ distribution__ ([#hypothtests]).
...
<caption> (\#tab:choosingsamplesit) esults from a 1998 study which surveyed 1000 people each from 5 countries
...
__2.__ The respondents could choose: 'agree' 'disagree' or 'don't know' to the statements in the table above. If we compared the proportion of Canadians who agreed "Recent changes will harm quality" with those Canadians who disagreed with that statement (which was 15\%), which sampling situation applies A, B or C?

+ __Situation B__ is appropriate. We have one sample of Canadians who either agree or disagree (or don't know) with this statement.
...
```{r, echo=T, eval=T}
matrixasbestos <- matrix (c(1,31,39,9), nrow=2, ncol=2, byrow=T) ###note order changed of variables
...
Using the above table, calculate the probability of being bald given cancer (i.e. Pr(bald|cancer) and the probability of being bald given no cancer.  
