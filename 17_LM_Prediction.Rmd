...
```{r, echo=T, eval=T}
# Fit a linear model
linearAll <- lm(density ~ X + Y + DistCoast + depth + as.factor(month) + phase,
                data=wfdata)
...
phase <- "C"
# Create dataframe
df.wf <- data.frame (X, Y, DistCoast, depth, month, phase)
# Prediction
predict(linearAll, newdata=df.wf, se.fit=TRUE, interval="confidence",
        level=0.95)
```

Hence, for the mean values of `X` and `Y`, a `DistCoast` of 5km and `depth` of 10m in `phase` C in April, the predicted density of birds is 4.69 (95\ 4.38-5.00) birds per km$^2$. 

It is possible to predict for any combinations of covariates even those that did not occur in the data. For example, the `depth` and `DistCoast` values used in the above prediction may not actually occur together for any record in the observed data. This is, of course, a form of extrapolation and is **dangerous** (and potentially even pointless - why extrapolate from combinations of variables that do not occur in nature?). 
