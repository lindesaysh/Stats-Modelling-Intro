...
+ In this case, the `Phase` term considered alone is significant. If it had not been significant, it would be still be retained in the model because it forms part of a significant interaction term.
+ There are no grounds to reduce this model further, if backwards selection was being undertaken. 
...
If backwards model selection is being undertaken, interaction terms should be considered first for removal (following the principle of marginality) and then other main effects, not associated with remaining interactions, as before. Main effects associated with the interaction should not be considered (for rejection) unless the interaction is rejected. 
...
AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
                     Phase + XPos:Phase + YPos:Phase, data=wfdata))
# remove YPos:phase interaction
AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
                     Phase + XPos:Phase, data=wfdata))
# remove XPos:phase interaction
AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
...
```{r, echo=F}
# Save AICs for models fitted previously
AIC.full <- AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
                     Phase + XPos:Phase + YPos:Phase, data=wfdata))
# Exclude YPos:Phase
AIC.intXPos <- AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
                     Phase + XPos:Phase, data=wfdata))
# Exclude XPos:Phase
AIC.intYPos <- AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) +  
                     Phase + YPos:Phase, data=wfdata))
```

The AICs are `r format(AIC.full)`, `r format(AIC.intXPos)` and  `r format(AIC.intYPos)`, respectively; the smallest AIC is for the model without the the `YPos:Phase` term (perhaps not surprisingly given the model chosen using $p$-values). We now use this model (i.e. excluding `YPos:Phase`) and consider how removing other covariates (e.g. `DistCoast`, `Depth` and `Month`) affect the AIC. 
...
AIC(lm(Density ~ XPos + YPos + DistCoast + as.factor(Month) + 
          XPos:Phase, data=wfdata))
# remove Month
AIC(lm(Density ~ XPos + YPos + DistCoast + Depth + XPos:Phase, 
        data=wfdata))
```
\normalsize
...
```{r, echo=F, eval=T}
# remove Distcoast
AIC.m1 <- AIC (lm(Density ~ XPos + YPos + Depth + as.factor(Month) + 
          XPos:Phase, data=wfdata))
# remove Depth
AIC.m2 <- AIC (lm(Density ~ XPos + YPos + DistCoast + as.factor(Month) + 
          XPos:Phase, data=wfdata))
# remove Month
AIC.m3 <- AIC (lm(Density ~ XPos + YPos + DistCoast + Depth + 
          XPos:Phase, data=wfdata))
```

The AICs are `r format(AIC.m1)`, `r format(AIC.m2)`, and `r format(AIC.m3)`, respectively, so we have no grounds to reduce the model further. Our best model using AIC is:

`Density ~ XPos + YPos + DistCoast + Depth + as.factor(Month) + Phase + XPos:Phase`. 

In this example, using $p$-values and AICs for model selection have resulted in the same model being selected. This may not always be the case:
